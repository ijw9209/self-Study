...
재사용 가능한 여러 컴포넌트를 만들어 놓는것은 더큰 앱을 만들때 두각을 나타냅니다.
UI일부가 여러번 사용되거나(Button, Panel , Avatar) , UI 일부가 자체적으로 복잡한경우
재사용 가능한 컴포넌트로 만드는 것이 좋다.
...
이런 함수들은 **순수함수**라고 호칭합니다. 입력값을 바꾸려하지 않고 항상 동일한 입력값에 대해 동일한 결과를 반환하기 때문이다

반면에 아래 함수는 자신의 입력값을 변경하기때문에 순수함수가 아닙니다.
...
```
React는 매우 유연하지만 한가지 규칙이있습니다

모든 React 컴포넌트는 자신의 props를 다룰때 반드시 순수함수처럼 동작해야합니다.
물론 애플리케이션 UI는 동적이며 시간에 따라 변한다.
그래서 React 컴포넌트는 state를 통해 위 규칙을 위반하지않고 사용자 액션, 네트워크 응답 및 다른 요소에 대한 응답으로 시간에따라 자신의 출력값을 변경할 수 있습니다